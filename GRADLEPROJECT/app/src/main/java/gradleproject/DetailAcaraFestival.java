...
public class DetailAcaraFestival {
    ...
    public DetailAcaraFestival() {
        ...
        // Tab Oranye di atas
        Label lblTab = new Label("Festival");
        ...
        try {
            // Sesuaikan path ini dengan lokasi gambar aslimu di folder resources
            Image img = new Image(getClass().getResourceAsStream("/aset/gambarLuminara/img-fest-detail.png"));
            imgEvent.setImage(img);
        } catch (Exception e) {
            System.out.println("⚠️ Gambar acara tidak ditemukan.");
        }
        imgEvent.setFitWidth(720);
        imgEvent.setFitHeight(200); // Tinggi gambar
        
        // Memotong sudut atas gambar agar melengkung rapi mengikuti kartu putih
        ...
        textInfoBox.setPadding(new Insets(10, 25, 25, 25)); // Padding dalam untuk teks
        ...
        Label lblDesc = new Label("Pementasan yang menampilkan busana adat Makassar dan Sulawesi Selatan. Acara ini menonjolkan Baju Bodo, Baju Bella Dada, passapu, serta kain sutra dan aksesoris tradisional sebagai simbol identitas, status sosial, dan filosofi budaya dalam bentuk parade atau runway budaya.");
        lblDesc.setWrapText(true); // Teks akan turun ke bawah otomatis jika terlalu panjang
        lblDesc.setStyle("-fx-font-family: 'Poppins'; -fx-font-size: 12px; -fx-text-fill: #1A3C5A; -fx-line-spacing: 0.5em;");

        // --- GRID SPESIFIKASI (Lokasi, Tanggal, dll) ---
        GridPane gridInfo = new GridPane();
        gridInfo.setHgap(150); // Jarak antar kolom
        gridInfo.setVgap(15);  // Jarak antar baris
