...
public class ProfilUser {
 ...
 public ProfilUser(boolean showSaveButton) {
  ...
  // =====================================================================
  // 1. MENARIK DATA DARI DATABASE
  // =====================================================================
  String namaUser = "Pengguna";
  String inisialUser = "P";
  String emailUser = "";
  String teleponUser = "";
  String passwordUser = "";
  ...
  if (UserSession.getInstance() != null) {
   ...
   if (currentUser != null) {
    if (currentUser.getUsername() != null && !currentUser.getUsername().isEmpty()) {
     namaUser = currentUser.getUsername();
     // Ambil huruf pertama dari nama untuk Avatar
     inisialUser = String.valueOf(namaUser.charAt(0)).toUpperCase(); 
    }
    emailUser = currentUser.getEmail() != null ? currentUser.getEmail() : "";
    teleponUser = currentUser.getPhoneNumber() != null ? currentUser.getPhoneNumber() : "";
    passwordUser = currentUser.getPassword() != null ? currentUser.getPassword() : "";
   }
  }

  // =====================================================================
  // 2. STRUKTUR BANNER & AVATAR OVERLAPPING
  // =====================================================================
  StackPane headerLayout = new StackPane();
  headerLayout.setAlignment(Pos.TOP_LEFT);
  headerLayout.setPrefHeight(220); 
  ...
  // 👉 Menampilkan Inisial Asli User
  Label lblInitial = new Label(inisialUser); 
  ...
  StackPane.setMargin(avatarCircle, new Insets(0, 0, 0, 0)); 

  headerLayout.getChildren().addAll(ivBanner, avatarCircle);

  // =====================================================================
  // 3. BLOK DESKRIPSI RINGKAS & PARAMETER DATA
  // =====================================================================
  ...
  // =====================================================================
  // 4. SEKSYEN FORMULIR INFORMASI (Diisi dari Database)
  // =====================================================================
  VBox sectionForm = new VBox(15);
  
  Label lblSectionTitle = new Label("Informasi Akun Kamu");
  ...
  // 👉 Memasukkan data asli ke dalam TextField
  txtNama = createStyledTextField(namaUser);
  txtNama.setEditable(showSaveButton); // Kunci form jika hanya melihat profil
  VBox fieldNama = createFieldBlock("Nama", txtNama, showSaveButton); 
  gridForm.add(fieldNama, 0, 0);

  txtEmail = createStyledTextField(emailUser);
  txtEmail.setEditable(showSaveButton);
  VBox fieldEmail = createFieldBlock("Email", txtEmail, showSaveButton);
  gridForm.add(fieldEmail, 1, 0);

  txtTelepon = createStyledTextField(teleponUser);
  txtTelepon.setEditable(showSaveButton);
  VBox fieldTelepon = createFieldBlock("No Telepon", txtTelepon, showSaveButton);
  gridForm.add(fieldTelepon, 0, 1);

  // Password disamarkan jika hanya melihat profil, ditampilkan jika sedang mode edit
  String displayPassword = showSaveButton ? passwordUser : "••••••••";
  txtPassword = createStyledTextField(displayPassword); 
  txtPassword.setEditable(showSaveButton);
  VBox fieldPassword = createFieldBlock("Password", txtPassword, showSaveButton);
  gridForm.add(fieldPassword, 1, 1);

  GridPane.setHgrow(fieldNama, Priority.ALWAYS);
  GridPane.setHgrow(fieldEmail, Priority.ALWAYS);

  // ... (setelah gridForm ditambahkan ke sectionForm)
sectionForm.getChildren().addAll(lblSectionTitle, gridForm);

// Pastikan pengkondisian ini ada dan menambahkan ke sectionForm
if (showSaveButton) {
    ...
    sectionForm.getChildren().add(actionRow); // <--- INI WAJIB ADA
}

// Pastikan sectionForm ditambahkan ke profileContentBox
